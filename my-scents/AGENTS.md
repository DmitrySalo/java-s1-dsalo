...
1. Прочитай затронутый код, тесты и конфигурацию, не делай предположений по шаблонам другого проекта.
2. Прочитай `.opencode/instructions/architecture.md` для любого изменения структуры, сценария или интеграции.
3. Прочитай дополнительные инструкции по области изменения:
...
Перед завершением выполни релевантные тесты, сообщи точные команды и результат, добавь изменения в Git без commit. При изменении архитектуры обнови `ARCHITECTURE.md`; при изменении пользовательского поведения/публичного контракта обнови `README.md`; всегда добавь запись в `REPORT.md` с датой и ссылкой на prompt задания.
